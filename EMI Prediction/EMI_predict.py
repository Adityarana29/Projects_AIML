...
dt_reg = f"{MODEL_DIR}/dt_regressor.pkl"
...
def train_or_load(df):
    ...
    if all(os.path.exists(p) for p in [
        CLS_PATH, REG_PATH, SCALER_PATH, TARGETENC_PATH, FEATURELIST_PATH, ENCODERS_PATH,
        f"{MODEL_DIR}/dt_regressor.pkl"
    ]):
        return (
            load(CLS_PATH),
            load(REG_PATH),
            load(f"{MODEL_DIR}/dt_regressor.pkl"),
            load(SCALER_PATH),
            load(TARGETENC_PATH),
...
if predict:
    ...
    input_df = pd.DataFrame([user_inputs])  # ✅ Direct from sidebar
    ...
    reg_pred_tree = dt_reg.predict(input_df)[0]
