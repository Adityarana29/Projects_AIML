...
def train_or_load(df):
    ...
    X_train, X_test, ycls_train, ycls_test = train_test_split(
        X, y_cls, test_size=0.25, random_state=42
    )
    ...
    return cls_model, reg_model, scaler, le_target, feature_list, encoders


cls_model, reg_model, scaler, le_target, feature_list, encoders = train_or_load(df)
...
if predict:
    ...
    with c2:
        st.info(f"💰 Estimated EMI Capacity: **₹ {reg_pred:,.2f}**")
